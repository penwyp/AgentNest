...
- **目录数据**：`AgentDefinition.marketplace`（可选字段：`summary`、`homepageURL`、`install { kind: brew|cask, formula }`），catalog 校验白名单同步；市场列出全部目录产品（含未参与扫描的产品），按名称排序。当前目录 15 个产品：Claude Code (CLI)/(Desktop)、Codex (CLI)/(Desktop)、Cursor (CLI)/(Desktop)、Trae、Grok、Pi、Antigravity (Desktop)/(CLI)、OpenCode、Aider、ChatGPT (Desktop)、WorkBuddy；其中 Grok / Pi / Codex (Desktop) 无可验证安装方式 → 「暂无安装方式」+ 主页链接（brew 的 `grok` 公式是 Logstash 库，不可误用）。
